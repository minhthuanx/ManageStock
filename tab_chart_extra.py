...
def render_extra(df, bulk_df, bulk_history, sold_df, pbd, has_data):
    ...
    with st.container(border=True):
        ...
        if _sk_src:
            _mut_palette = ["#818cf8","#34d399","#f472b6","#a78bfa","#fbbf24",
                            "#fb7185","#38bdf8","#ff6b35","#4ade80","#f9a8d4"]
            _sk_node_colors = (
                ["#0088aa"]
    ...
    with st.container(border=True):
        ...
        if has_data and not pbd.empty:
            ...
            fig_cal = go.Figure(go.Heatmap(
                ...
                zmax=_zmax_cal,
                showscale=True,
                colorbar=dict(thickness=12, len=0.8, tickfont=dict(size=9, color="#94a3b8")),
                xgap=3, ygap=3,
            ))
    ...
    with st.container(border=True):
        ...
        if has_data and not pbd.empty:
            ...
            if len(_wk_merged) >= 1:
                ...
                _trend_color = "#34d399" if _m_coef >= 0 else "#fb7185"
                _trend_label = f"Xu hướng {'↑ tăng' if _m_coef >= 0 else '↓ giảm'} {abs(_m_coef / max(abs(_y.mean()), 1) * 100):.1f}%/tuần"

                _bar_colors = ["#34d399" if v >= 0 else "#fb7185" for v in _wk_merged["Lợi Nhuận"]]
                _fig_wk = go.Figure()
                ...
                _fig_wk.update_layout(
                    ...
                    xaxis=dict(tickfont=dict(color="#f1f5f9", size=10), gridcolor="#111111"),
                    yaxis=dict(title="Lợi nhuận (₫)", gridcolor="#111111",
                    ...
                    yaxis2=dict(title="Số con", overlaying="y", side="right",
                                tickfont=dict(color="#818cf8", size=10), zeroline=False, showgrid=False),
        ...
        if not sold_df.empty and "Số Trait" in sold_df.columns:
            ...
            with _trc1:
                ...
                _fig_tr.update_layout(
                    title=dict(text="Lợi Nhuận TB / Con theo Trait", font=dict(size=12, color="#f1f5f9")),
                    paper_bgcolor="#000000", plot_bgcolor="#000000",
                    font=dict(family="Inter", color="#94a3b8", size=11),
                    xaxis=dict(gridcolor="#111111", tickfont=dict(color="#f1f5f9", size=10)),
                    yaxis=dict(gridcolor="#111111", tickfont=dict(color="#94a3b8", size=10),
    ...
    with st.container(border=True):
        ...
        _fig_gauge = go.Figure(go.Indicator(
            ...
            value=_recovery_pct,
            delta={"reference": 80, "suffix": "%", "increasing": {"color": "#34d399"}, "decreasing": {"color": "#fb7185"}},
            ...
            title={"text": "% Vốn Đã Thu Hồi", "font": {"size": 12, "color": "#94a3b8", "family": "Inter"}},
        ))
