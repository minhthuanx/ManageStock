# =============================================================================
# TAB KHỐ LE — NHẬP TAY NHIỀU DÒNG (manual multi-row)
# Form nhập tay giống hệt dialog JSON Import (tab_kho_json) nhưng điền tay:
#   - Nút "+ Thêm dòng" để thêm số lượng dòng tùy ý
#   - Mỗi dòng: Tên Pet, Mutation, M/s, Số Trait, NameStock, Giá nhập,
#     Giá bán ($) + ảnh listing (để push Eldorado nếu đủ điều kiện)
# =============================================================================
...
def render_manual_multi(df, pet_db, ns_db, trait_db, eld_client=None):
    """Render multi-row manual entry form (giống dialog JSON nhưng nhập tay)."""

    # ── State: số dòng đang hiển thị ──
    if "manual_rows" not in st.session_state:
        st.session_state.manual_rows = 1
    # ── State: ảnh đã upload giữa các rerun ──
    if "manual_img_bytes" not in st.session_state:
        st.session_state.manual_img_bytes = {}

    with st.container(border=True):
        st.markdown("**Nhập Tay Nhiều Dòng**")
        st.caption("Điền tay từng dòng — thêm/sớt dòng bằng nút bên dưới · "
                   "Dòng có ảnh + giá bán $ ≥ 0.50 sẽ tự push lên Eldorado")

        # ── NameStock chung cho cả batch (tùy chọn) ──
        ns_opts = [""] + get_name_options(ns_db, fallback="")
        _gn1, _gn2 = st.columns([1, 3])
        use_global_ns = _gn1.checkbox("NameStock chung", key="manual_global_ns_check",
                                       help="Áp dụng cùng 1 NameStock cho tất cả dòng")
        if use_global_ns:
            global_ns_val = _gn2.selectbox(
                "NameStock áp dụng cho tất cả", ns_opts,
                key="manual_global_ns_val", label_visibility="collapsed",
            )
        else:
            global_ns_val = ""
        ...
        edited_rows = []
        row_errs = {}
        n_rows = st.session_state.manual_rows

        pet_opts = get_name_options(pet_db)
        pet_opts_lower = set(x.lower() for x in pet_opts)
        trait_opts = ["None"] + [str(n) for n in range(1, 16)]

        for row_i in range(n_rows):
            with st.container(border=True):
                # Header dòng + nút xoá
                _h1, _h2 = st.columns([5, 0.8])
                _h1.markdown(f"<b>Dòng {row_i + 1}</b>", unsafe_allow_html=True)
                if n_rows > 1:
                    if _h2.button("×", key=f"manual_del_row_{row_i}",
                                  help="Xoá dòng này", use_container_width=True):
                        # Xoá các widget key của dòng bị xoá + ảnh
                        st.session_state.pop(f"manual_name_{row_i}", None)
                        st.session_state.pop(f"manual_ms_{row_i}", None)
                        st.session_state.pop(f"manual_mut_{row_i}", None)
                        st.session_state.pop(f"manual_trait_{row_i}", None)
                        st.session_state.pop(f"manual_ns_{row_i}", None)
                        st.session_state.pop(f"manual_cost_{row_i}", None)
                        st.session_state.pop(f"manual_price_{row_i}", None)
                        st.session_state.pop(f"manual_img_{row_i}", None)
                        st.session_state.manual_img_bytes.pop(f"manual_img_{row_i}", None)
                        st.session_state.manual_rows -= 1
                        st.rerun()

                col1, col2, col3 = st.columns(3)

                # Tên Pet (selectbox + cho phép nhập tên mới qua expander)
                pet_sel = col1.selectbox(
                    f"Tên Pet *", pet_opts, key=f"manual_name_{row_i}",
                    label_visibility="collapsed",
                    placeholder="Chọn pet...",
                )
                # Mutation
                _norm_i = next((j for j, m in enumerate(MUTATION_OPTIONS) if m == "Normal"), 0)
                mut_sel = col2.selectbox(
                    f"Mutation", MUTATION_OPTIONS, index=_norm_i,
                    key=f"manual_mut_{row_i}", label_visibility="collapsed",
                )
                # M/s
                ms_raw = col3.text_input(
                    f"M/s *", placeholder="VD: 975", key=f"manual_ms_{row_i}",
                    label_visibility="collapsed",
                )
                ...
                trait_sel = col4.selectbox(
                    f"Số Trait", trait_opts, key=f"manual_trait_{row_i}",
                    label_visibility="collapsed",
                )
                # NameStock
                if use_global_ns:
                    ...
                    ns_val = col5.selectbox(
                        f"NameStock *", ns_opts, key=f"manual_ns_{row_i}",
                        label_visibility="collapsed",
                    )
                # Giá nhập
                cost_raw = col6.text_input(
                    f"Giá nhập *", placeholder="150k / 1.5tr / 1500000",
                    key=f"manual_cost_{row_i}", label_visibility="collapsed",
                )
                ...
                if _show_push:
                    ...
                    _saved_key = f"manual_img_{row_i}"
                    ...
                    if _saved:
                        ...
                        if _img_col.button("Xóa ảnh", key=f"manual_rm_img_{row_i}",
                        ...
                        up_img = _img_col.file_uploader(
                            f"Ảnh listing (để push Eldorado)",
                            type=["png", "jpg", "jpeg", "webp"],
                            key=f"manual_img_{row_i}", label_visibility="collapsed",
                        )
                        if up_img:
                            _b = up_img.read()
                            up_img.seek(0)
                    ...
                    price_raw = _price_col.text_input(
                        f"Giá bán ($)", placeholder="$0.50", key=f"manual_price_{row_i}",
                        label_visibility="collapsed",
                    )
                else:
                    r_img = None
                ...
                r_title = st.text_input(
                    ...
                    key=f"manual_title_{row_i}", label_visibility="collapsed",
                )

            # ── Validate + thu thập dữ liệu dòng ──
        ...
        if _b1.button("＋ Thêm dòng", use_container_width=True, key="btn_manual_add"):
            st.session_state.manual_rows += 1
            st.rerun()
        if n_rows > 1:
            if _b2.button("－ Bớt dòng", use_container_width=True, key="btn_manual_remove"):
                st.session_state.manual_rows -= 1
                st.rerun()
