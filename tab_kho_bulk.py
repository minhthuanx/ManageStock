        ...
        with st.expander("Giao dịch hàng loạt", expanded=False):
            ...
            if _bs_df.empty and _bs_search.strip():
                ...
                for _, _br in _shown_bs.iterrows():
                    ...
                    _br_ns_str = f" · <span style='color:#6a6a70'>{_br_ns}</span>" if _br_ns else ""
                    ...
                    _rc1.markdown(
                        ...
                        f'<b style="color:#ff8533">#{int(_br["STT"])}</b> · '
                        f'<b>{_br["Tên Pet"]}</b> · <span style="color:#ff8533">{_br["Mutation"]}</span>'
                        f'{_br_ms_str}{_br_ns_str}{_br_trait_str}'
                        f' · <span style="color:#9a8a7a">{fmt_vnd(float(_br["Giá Nhập"]))}</span>'
                        f'{_br_ton_str}'
                        f'</div>',
...
def render_resell(df):
    ...
    with st.expander("🔄 Bán lại (Re-sell)", expanded=False):
        ...
        st.markdown(
            ...
            'color:#ff6a00;font-weight:600;">📌 Danh sách Pin</span>'
            '<span style="font-size:0.72rem;color:#9a8a7a;">— nhấn Re-sell khi chắc chắn khách không lấy</span>'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        if not st.session_state.pinned_resell and not st.session_state.resell_cart:
            st.markdown(
                ...
                'font-size:0.82rem;border:1px dashed rgba(255,106,0,0.2);border-radius:8px;">'
            ...
            for _pid in list(st.session_state.pinned_resell.keys()):
                ...
                _pv_ns_str  = f" · <span style='color:#6a6a70'>{_pv_ns}</span>" if _pv_ns else ""
                ...
                st.markdown(
                    ...
                    f'<b>{_pv.get("Tên Pet", "")}</b> · <span style="color:#ff8533">{_pv_mut}</span>'
                ...
                if _pc2.button("❌ Bỏ pin", key=f"rs_del_{_pid}", use_container_width=True):
                    ...
                    st.rerun()
                st.markdown('<div style="border-top:1px solid rgba(255,106,0,0.12);margin:2px 0 4px 0;"></div>', unsafe_allow_html=True)
            ...
            if _resell_src.empty:
                ...
                if not _rs_search.strip():
                    ...
                    for _, _rr in _shown_rs.iterrows():
                        ...
                        _rr_ns_str  = f" · <span style='color:#6a6a70'>{_rr_ns}</span>" if _rr_ns else ""
                        ...
                        st.markdown(
                            ...
                            f'<b>{_rr["Tên Pet"]}</b> · <span style="color:#ff8533">{_rr["Mutation"]}</span>'
        ...
        if st.session_state.resell_cart:
            ...
            st.markdown(
                ...
                '<span style="font-size:0.72rem;color:#9a8a7a;">— tạo bản ghi kho mới, giá nhập 1₫</span>'
                '</div>',
                unsafe_allow_html=True,
            )
