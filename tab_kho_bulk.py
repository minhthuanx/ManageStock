            ...
            if _bs_df.empty and _bs_search.strip():
                ...
                for _, _br in _shown_bs.iterrows():
                    ...
                    _rc1.markdown(
                        ...
                        f'<b style="color:#a1a1aa">#{int(_br["STT"])}</b> · '
                        f'<b>{_br["Tên Pet"]}</b> · <span style="color:#a1a1aa">{_br["Mutation"]}</span>'
...
def render_resell(df):
    ...
    with st.expander("🔄 Bán lại (Re-sell)", expanded=False):
        ...
        if not st.session_state.pinned_resell and not st.session_state.resell_cart:
            ...
            for _pid in list(st.session_state.pinned_resell.keys()):
                ...
                st.markdown(
                    ...
                    f'<b>{_pv.get("Tên Pet", "")}</b> · <span style="color:#a1a1aa">{_pv_mut}</span>'
                ...
                if _pc2.button("❌ Bỏ pin", key=f"rs_del_{_pid}", use_container_width=True):
                    ...
                    st.rerun()
                st.markdown('<div style="border-top:1px solid #1f1f1f;margin:2px 0 4px 0;"></div>', unsafe_allow_html=True)
            ...
            if _resell_src.empty:
                ...
                if not _rs_search.strip():
                    ...
                    for _, _rr in _shown_rs.iterrows():
                        ...
                        st.markdown(
                            f'<div style="font-size:0.82rem;padding:4px 0 2px 0;">'
                            f'<b style="color:#a1a1aa">#{int(_rr["STT"])}</b> · '
                            f'<b>{_rr["Tên Pet"]}</b> · <span style="color:#a1a1aa">{_rr["Mutation"]}</span>'
        ...
        if st.session_state.resell_cart:
            ...
            st.markdown(
                '<div style="display:inline-flex;align-items:center;gap:6px;'
                'background:rgba(161,161,170,0.1);border:1px solid rgba(161,161,170,0.3);'
                'border-radius:6px;padding:4px 10px;margin-bottom:6px;">'
                '<span style="font-size:0.72rem;letter-spacing:0.06em;text-transform:uppercase;color:#a1a1aa;font-weight:600;">③ Xác nhận Re-sell</span>'
