    ...
    with st.container(border=True):
        ...
        if not sold_df.empty:
            ...
            if not _hour_df.empty:
                ...
                _colors = ["#bc5090" if h == _peak_hour else "#0088aa" for h in _hour_count["Giờ"]]
                ...
                fig_hour = go.Figure(go.Bar(
                    ...
                    textfont=dict(size=11, color="#e8e0e8"),
                ))
                fig_hour.update_layout(
                    ...
                    xaxis=dict(tickangle=-45, tickfont=dict(size=10, color="#9aafc4")),
                    yaxis=dict(tickfont=dict(size=10, color="#9aafc4"), gridcolor="#003f5c"),
                    height=430,
                )
        ...
        if not sold_df.empty:
            ...
            if not _hmap_df2.empty:
                ...
                fig_hmap.update_layout(
                    ...
                    height=380,
                    xaxis=dict(tickangle=-45, tickfont=dict(size=10, color="#9aafc4")),
                    yaxis=dict(tickfont=dict(size=10, color="#9aafc4")),
                )
    ...
    with st.container(border=True):
        ...
        for _ai, (_an, _ab) in enumerate(_ach_display):
            ...
            _ach_track_cols[_ai].markdown(
                ...
                f"background:{'rgba(255,166,0,0.12)' if _done else 'rgba(255,255,255,0.03)'};"
                f"border:1px solid {'rgba(255,166,0,0.3)' if _done else 'rgba(255,255,255,0.06)'};'>"
                f"<div style='font-size:1.1rem;'>{'✅' if _done else '⬜'}</div>"
                f"<div style='font-size:0.65rem;color:{'#ffa600' if _done else '#6b8a9e'};font-weight:600;'>{_ab}</div>"
                f"<div style='font-size:0.6rem;color:#6b8a9e;'>{_an}</div></div>",
        ...
        for _ms_row in _ms_rows:
            ...
            for _ci, _ms in enumerate(_ms_row):
                ...
                _row_cols[_ci].markdown(
                    ...
                    f"background:{'rgba(255,166,0,0.12)' if _done else ('rgba(188,80,144,0.10)' if _is_next else 'rgba(255,255,255,0.03)')};"
                    f"border:1px solid {'rgba(255,166,0,0.3)' if _done else ('rgba(188,80,144,0.3)' if _is_next else 'rgba(255,255,255,0.06)')};"
                    f"{'font-weight:700;' if _is_next else ''}'>"
                    f"<div style='font-size:0.75rem;color:{'#ffa600' if _done else ('#bc5090' if _is_next else '#6b8a9e')};'>"
